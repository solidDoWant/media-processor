...
5. The worker picks up the job: it probes the file, detects black-bar crop, transcodes to H.265 MKV, and writes the output to `/processed-output` (mirroring the input's subdirectory when `MEDIA_INPUT_ROOT` is set).
6. The worker calls the Radarr or Sonarr API to trigger a `DownloadedMoviesScan` / `DownloadedEpisodesScan` for the output path.
...
Integration tests against a live Hatchet instance:

```sh
make hatchet-up         # start local Hatchet containers and generate an API token
