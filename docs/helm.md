...
| `config.watcher.searchAttributes.enabled`  | bool   | `true`      | When `true`, sets `WATCHER_TEMPORAL_SEARCH_ATTRIBUTES_ENABLED=true` on the watcher (the default). When `false`, sets it to `false` to disable custom search attributes. Requires the five custom search attributes to be pre-registered in the Temporal namespace when `true`; see [Temporal search attributes](configuration.md#temporal-search-attributes) for the registration command. |
