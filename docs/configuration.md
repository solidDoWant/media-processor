...
The default schedule (5s, 7.5s, 11s, 17s, 25s, 38s, 57s, then 60s × 8 attempts ≈ 9 minutes total wall time) is tuned for the most common failure mode: NFS attribute-cache staleness on the arr side. When the arr service mounts the media volume over NFS, the kernel client caches directory entries for up to `acdirmax` seconds (default `60s`); a scan issued immediately after the file is written may see the stale cache and report no successful import. The early retries (5–17s) cover transient arr command-pool saturation; the tail (capped at 60s) extends past a typical NFS cache window so at least one attempt sees the refreshed dirent.
