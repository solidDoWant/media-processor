...
| `TEMPORAL_API_KEY`                       | string       | `""`                                        | Optional     | API key for authenticated Temporal frontends (e.g. Temporal Cloud). Overrides `api_key` in the loaded profile. Sent as `Authorization: Bearer <key>` on every RPC.                                                                                                                       |
