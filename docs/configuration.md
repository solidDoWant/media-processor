...
| `WORKER_STOP_TIMEOUT`             | duration       | `4h`    | Optional     | On SIGTERM, the worker stops accepting new Temporal tasks and waits up to this long for in-flight activities to finish before cancelling them. Defaults to the `MEDIA_TRANSCODE_TIMEOUT` default (`4h`) so the longest expected activity has a chance to complete before the worker exits.                                                                                                                      |
