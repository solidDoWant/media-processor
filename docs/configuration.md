...
| `MEDIA_NOTIFY_INITIAL_INTERVAL`   | duration | `5s`                                 | Optional     | First retry interval for the Notify activity (Temporal `RetryPolicy.InitialInterval`). Must be a positive Go duration. See [Notify retry policy](#notify-retry-policy) for tuning guidance.                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `MEDIA_NOTIFY_BACKOFF_COEFFICIENT`| float    | `1.5`                                | Optional     | Growth factor applied between successive Notify retries (Temporal `RetryPolicy.BackoffCoefficient`). Must be `>= 1.0`; `1.0` means constant spacing, larger values mean exponential growth capped by `MEDIA_NOTIFY_MAXIMUM_INTERVAL`. See [Notify retry policy](#notify-retry-policy).                                                                                                                                                                                                                                                                                                                                                           |
| `MEDIA_NOTIFY_MAXIMUM_INTERVAL`   | duration | `60s`                                | Optional     | Cap on the spacing between Notify retries (Temporal `RetryPolicy.MaximumInterval`). Must be a positive Go duration. See [Notify retry policy](#notify-retry-policy).                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `MEDIA_NOTIFY_MAXIMUM_ATTEMPTS`   | integer  | `15`                                 | Optional     | Total Notify attempts before the activity gives up (Temporal `RetryPolicy.MaximumAttempts`). Must be a positive integer. See [Notify retry policy](#notify-retry-policy).                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
The Notify activity issues the Sonarr/Radarr scan command after a successful transcode and blocks until the import command reaches a terminal state. It is the last activity in the workflow before Cleanup, and a `Result="unsuccessful"` from the arr command (the file was not picked up by the scan) returns a retryable error. The four `MEDIA_NOTIFY_*` variables are the four fields of the Temporal `RetryPolicy` applied to this activity.

There is one exception that is **not** retried: when the scan reports no successful import and the arr service can no longer match the file to a library item — the movie or series was deleted, or the episode is no longer monitored — the import can never succeed. The workflow treats this as a benign skip rather than a failure: it logs the event, increments `media_workflow_import_skipped_not_in_library_total`, removes the orphaned transcoded output file (which a successful import would normally have consumed), and completes successfully without retrying or firing the failure webhook.

The default schedule (5s, 7.5s, 11s, 17s, 25s, 38s, 57s, then 60s × 7 across 15 attempts ≈ 9–10 minutes total wall time) is tuned for the most common failure mode: NFS attribute-cache staleness on the arr side. When the arr service mounts the media volume over NFS, the kernel client caches directory entries for up to `acdirmax` seconds (default `60s`); a scan issued immediately after the file is written may see the stale cache and report no successful import. The early retries (5–17s) cover transient arr command-pool saturation; the tail (capped at 60s) extends past a typical NFS cache window so at least one attempt sees the refreshed dirent.

Tuning notes:

- Faster retries below `acdirmax` do not help the NFS-cache failure mode; they only help arr-side transient errors. Lowering `MEDIA_NOTIFY_INITIAL_INTERVAL` further mostly burns budget.
- The activity's `StartToCloseTimeout` is `1h` per attempt. The total wall-clock ceiling is `MEDIA_NOTIFY_MAXIMUM_ATTEMPTS × 1h` (per-attempt timeout) plus the sum of inter-attempt intervals, but in practice each attempt finishes in seconds and the inter-attempt intervals dominate.
- `MEDIA_NOTIFY_BACKOFF_COEFFICIENT=1.0` produces constant spacing equal to `MEDIA_NOTIFY_INITIAL_INTERVAL`; useful when you want predictable polling.
- Each Notify attempt re-issues the scan command, which the arr service queues in its 3-thread executor. Aggressive retries against a busy arr instance can amplify the queue; if you run many parallel imports, prefer growing the backoff coefficient over raising the attempt count.
