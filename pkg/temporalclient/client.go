...
// apiKeyFilePrefix marks an API key value as a path to a file containing the
// key. Only the canonical empty-authority + absolute-path form is accepted.
const apiKeyFilePrefix = "file://"
...
func buildOptions() (client.Options, error) {
	...
	if apiKeyFile != "" {
		if _, err := os.ReadFile(apiKeyFile); err != nil {
			return client.Options{}, fmt.Errorf("validate api key file %q: %w", apiKeyFile, err)
		}
