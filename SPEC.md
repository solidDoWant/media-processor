...
6. **Library import triggered.** The workflow's `notify` step calls `ArrLibrary.ImportByFilePath` with the original input file path (e.g., `/downloads/my-media-item/video.mkv`). The path is translated if necessary via `LocalPathPrefix`/`RemotePathPrefix` to produce the path as Sonarr/Radarr sees it (e.g., `/downloads/my-media-item/video.mkv`), then a `DownloadedMoviesScan` (Radarr) or `DownloadedEpisodesScan` (Sonarr) command is sent with that path. Because the download client originally reported this same path to Sonarr/Radarr, the arr service can match it to the queued download entry and trigger the normal import pipeline.
